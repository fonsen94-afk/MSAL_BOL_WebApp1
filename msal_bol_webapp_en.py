# msal_bol_webapp_en_fields_tables.py
...
# أسماء Styles فريدة
styles.add(ParagraphStyle(name='MyTitleGreen', fontName=base_font, fontSize=18, leading=20, alignment=1, textColor=green_color))
styles.add(ParagraphStyle(name='MyFieldLabelGreen', fontName=base_font, fontSize=10, leading=12, textColor=green_color))
styles.add(ParagraphStyle(name='MyFieldValueBlack', fontName=base_font, fontSize=10, leading=12, textColor=black_color))
...
if st.button("Generate PDF with QR Code"):
    ...
    # Title
    elements.append(Paragraph("<b>MSAL SHIPPING</b><br/><b>BILL OF LADING</b>", styles['MyTitleGreen']))
    elements.append(Spacer(1, 12))

    # Create separate table for each field
    for key, value in data.items():
        if key != "Tracking URL":
            field_table = Table([
                [Paragraph(key, styles['MyFieldLabelGreen']), Paragraph(value.replace("\n", "<br/>"), styles['MyFieldValueBlack'])]
            ], colWidths=[80*10, 110*10])
            field_table.setStyle(TableStyle([
                ('BOX', (0,0), (-1,-1), 0.5, green_color),
                ('VALIGN', (0,0), (-1,-1), 'TOP'),
            ...
            elements.append(Spacer(1, 6))  # مسافة بين كل جدول
    ...
    qr_buffer.seek(0)
    elements.append(Paragraph("<b>Tracking QR Code</b>", styles['MyFieldLabelGreen']))
