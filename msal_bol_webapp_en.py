...
light_gray = colors.HexColor("#555555")  # لون الخط الفاتح

styles.add(ParagraphStyle(name='MyTitle', fontName=base_font, fontSize=18, leading=20, alignment=1, textColor=light_gray))
styles.add(ParagraphStyle(name='MyFieldLabel', fontName=base_font, fontSize=10, leading=12, textColor=light_gray))
styles.add(ParagraphStyle(name='MyFieldValue', fontName=base_font, fontSize=10, leading=12, textColor=light_gray))
...
if st.button("Generate PDF with QR Code"):
    ...
    # Title
    elements.append(Paragraph("<b>MSAL SHIPPING</b><br/><b>BILL OF LADING</b>", styles['MyTitle']))
    elements.append(Spacer(1, 12))
    ...
    for key, value in data.items():
        if key != "Tracking URL":
            rows.append([Paragraph(f"<b>{key}</b>", styles['MyFieldLabel']), Paragraph(value.replace("\n", "<br/>"), styles['MyFieldValue'])])
    ...
    qr_buffer.seek(0)
    elements.append(Paragraph("<b>Tracking QR Code</b>", styles['MyFieldLabel']))
