# msal_bol_webapp_en.py (fixed styles)
...
styles.add(ParagraphStyle(name='MyTitle', fontName=base_font, fontSize=16, leading=18, alignment=1))
styles.add(ParagraphStyle(name='MyFieldLabel', fontName=base_font, fontSize=9, leading=11))
styles.add(ParagraphStyle(name='MyFieldValue', fontName=base_font, fontSize=9, leading=11))
...
if st.button("Generate PDF with QR Code"):
    ...
    for key, value in data.items():
        if key != "Tracking URL":
            rows.append([
                Paragraph(f"<b>{key}</b>", styles['MyFieldLabel']),
